...
public class ImageDecoder {
    ...
    private static final int PARTICLE_SPACE = 8;
    ...
    public static ParticleImage decode(BufferedImage image) {
        ...
        for(double x = 0; x < image.getWidth(); x++) {
            for(double y = 0; y < image.getHeight(); y++) {
                ...
                pixelParticles[i] = new PixelParticle(x / PARTICLE_SPACE, y / PARTICLE_SPACE, image.getRGB((int) x, (int) y), image.getWidth(), image.getHeight(), PARTICLE_SPACE);
                i++;
            }
        }
